...
function readNormalizedData(site = activeSite) {
  site = normalizeSite(site);
  ensureNormalizedSchema();
  const tasks = database.prepare("SELECT * FROM tasks WHERE site_id = ? AND state='active' ORDER BY rowid DESC").all(site).map(taskFromRow);
  const archive = database.prepare("SELECT * FROM tasks WHERE site_id = ? AND state='archive' ORDER BY rowid DESC").all(site).map(taskFromRow);
